        ...
        for row in airodump_reader:
            ...
            if bssid != "BSSID":
                ...
                if bssid in bssid_power_dict:
                    bssid_power_dict[bssid].append((power,speed, timestamp))
                else:
                    bssid_power_dict[bssid] = [(power, speed, timestamp)]
...
cursor.execute('''CREATE TABLE IF NOT EXISTS ap_speed_power (
...
for bssid, speed_timestamp_list in bssid_power_dict.items():
    for power, speed, timestamp in speed_timestamp_list:
        query = "INSERT INTO ap_speed_power (bssid, speed, power, timestamp) VALUES (%s, %s, %s, %s)"
        data = (bssid, speed, power, timestamp)
